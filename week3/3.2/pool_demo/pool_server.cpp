...
int main(int argc, char **argv) {
    ...
    while(true) {
        // Инициализируем Set - устанавливаем Master и все Slave-сокеты - для вызова poll
        ...
        poll(Set, SetSize,
            -1); // timeout - бесконечность

        // пробегаем массив дескрипторов сокетов просматривая для каких из них имеются события для обработки
        for(unsigned int i = 0; i < SetSize; i++) {
            // 
            if(Set[i].revents & POLLIN) {
                if(i) {     // есть событие для одного из SlaveSockets
                ...
                } else {    // есть событие для MasterSocket (индекс 0)
